...
const columnVariants = {
  ...
  concluida: "border-green-200 bg-green-50/50 hover:bg-green-50/70 dark:border-green-800 dark:bg-green-950/20 dark:hover:bg-green-950/30",
};
...
const headerVariants = {
  ...
  concluida: "text-green-700 dark:text-green-300",
};
...
export function Column({ id, title, tasks, onTaskClick }: ColumnProps) {
  ...
  return (
    <div className={cn(
      "flex min-h-125 w-full flex-col rounded-[1.75rem] border-2 bg-white/70 p-4 backdrop-blur-sm transition-all duration-200 dark:bg-slate-900/70",
      columnVariants[id],
      isOver && "ring-2 ring-sky-500/60 ring-offset-2 ring-offset-slate-50 dark:ring-sky-400/60 dark:ring-offset-slate-950"
    )}>
      <div className={cn("mb-4 flex items-center justify-between", headerVariants[id])}>
        <h3 className="font-semibold text-sm uppercase tracking-wide">{title}</h3>
        <span className="rounded-full bg-white/85 px-2 py-1 text-xs font-medium text-slate-700 dark:bg-slate-950/80 dark:text-slate-200">
          {tasks.length}
        </span>
      </div>
