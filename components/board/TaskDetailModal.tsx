...
import type { Tag, Task, User } from '@/lib/types'
import { extractBackendMessage } from '@/lib/utils'
...
export function TaskDetailModal({
  ...
  const projectName =
    ...
    (projectId ? `Projeto ${projectId}` : 'Sem projeto')
  ...
  async function handleUpdate() {
    ...
    try {
      ...
      const payload = {
        ...
        estimated_minutes: formData.estimated_minutes ?? task.estimated_minutes,

        project_id: task.project_id,

        assignees:
          formData.assignees ?? task.assignees?.map((user) => user.id) ?? [],
      ...
      useStore.getState().updateTask(task.id, {
        ...
        assignees: users.filter((user) => payload.assignees.includes(user.id)),

        tags: tags.filter((tag) => payload.tags.includes(tag.id)),
      })
      ...
      toast.error(
        extractBackendMessage(err) ?? 'Não foi possível atualizar a tarefa.',
      )
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      toast.error(
        extractBackendMessage(err) ?? 'Não foi possível deletar a tarefa.',
      )
    } finally {
      setIsDeleting(false)
    }
  ...
  return (
    <>
      <div className={modalOverlay}>
        <div
          className={`${modalPanel} relative max-h-[90vh] max-w-4xl overflow-y-auto`}
        >
          <button
            onClick={() => {
              setIsEditing(false)
          ...
          <div className="p-8 pt-20">
            ...
            <div className="space-y-6">
              ...
              {taskTags && taskTags.length > 0 && (
                <div>
                  <span
                    className={`mb-2 block text-sm font-medium ${mutedText}`}
                  >
                    Tags:
                  </span>
              ...
              <div>
                <span
                  className={`cursor-pointer text-sm font-medium ${mutedText}`}
                >
                  Usuários atribuídos:
                </span>
                {isEditing ? (
                          ...
                          .map((user) => (
                            ...
                            >
                              ...
                              <button
                                type="button"
                                onClick={() => {
                                  const updated = formData.assignees?.filter(
                                    (id) => id !== user.id,
                                  )
                                  setFormData({
                                    ...formData,
        ...
        {showDeleteModal && (
          ...
          >
            ...
            >
              <h3 className="text-lg font-semibold mb-2">Deletar tarefa</h3>
