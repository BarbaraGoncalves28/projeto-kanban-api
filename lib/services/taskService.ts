...
import type { CreateTaskPayload, Tag, Task, UpdateTaskPayload } from '../types'
...
function normalizeTaskStatusFromApi(
...
async function populateTaskTags(
  tasks: Task[],
  token?: string,
): Promise<Task[]> {
...
export const taskService = {
  ...
  async createTask(payload: CreateTaskPayload, token?: string): Promise<Task> {
    ...
    const response = await client.post(
      ...
      normalizeTaskMutationPayload(payload),
    )
    return normalizeTaskFromApi(response.data.data ?? response.data)
  },
